...
const allowedKeys = new Set(['name', 'description', 'from_email', 'from_email_overridable', 'from_name', 'from_name_overridable', 'reply_to', 'reply_to_overridable', 'cc', 'bcc', 'x_mailer', 'verp_hostname', 'verp_disable_sender_header', 'mailer_type', 'mailer_settings', 'namespace']);
...
async function _getByTx(tx, context, key, id, withPermissions, withPrivateData) {
    ...
    if (withPrivateData) {
        ...
        entity = await tx('send_configurations').where(key, id).select(
            ['id', 'name', 'cid', 'description', 'from_email', 'from_email_overridable', 'from_name', 'from_name_overridable', 'reply_to', 'reply_to_overridable', 'cc', 'bcc']
        ).first();
