...
CONF=~/mailtrain_install.txt

set -e

export DEBIAN_FRONTEND=noninteractive

#just in case of multiple launch of this script
if [ -f ${CONF} ]; then
    . ${CONF}
else 
    ...
    echo "MYSQL_ROOT_PASSWORD=$MYSQL_ROOT_PASSWORD" > ${CONF}
    echo "MYSQL_PASSWORD=$MYSQL_PASSWORD"           >> ${CONF}
    echo "MYSQL_RO_PASSWORD=$MYSQL_RO_PASSWORD"     >> ${CONF}
    echo "DKIM_API_KEY=$DKIM_API_KEY"               >> ${CONF}
    echo "SMTP_PASS=$SMTP_PASS"                     >> ${CONF}
fi
...
apt-get -q -y install mariadb-server pwgen nodejs imagemagick git ufw build-essential dnsutils python software-properties-common npm redis-server
